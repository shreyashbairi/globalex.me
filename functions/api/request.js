...
export async function onRequestPost(ctx) {
  ...
  if (g.ip && (await rateLimited(db, g.ip, 6, 60 * 60 * 1000))) {
    return bad(
      "That is a lot of requests. Email contact@globalex.me and we will send everything at once.",
      429,
    );
  }
  ...
  try {
    ...
    return bad(
      "We saved your request but could not send the email just now. The desk will follow up, or write to contact@globalex.me.",
      424,
    );
  }
  ...
  ctx.waitUntil(
    (async () => {
      try {
        ...
        await send(env, {
          to: env.DESK_EMAIL || "contact@globalex.me",

...
const POLYMER_ORIGINS = ['Turkmenistan', 'Uzbekistan', 'UAE', 'Saudi Arabia', 'China'];

const POLYMERS = [
  {
    name: 'Polyethylene', kind: 'Resin family', vis: 'pe',
  ...
  {
    name: 'Polypropylene', kind: 'Resin family', vis: 'pp',
  ...
  {
    name: 'Performance additives', kind: 'Resin modifiers', vis: 'add',
    specs: ['Plasticizers', 'UV stabilizers', 'Flame retardants', 'Antimicrobials'],
  ...
  ...p, id: slug(p.name), cat: 'polymers', origins: POLYMER_ORIGINS,
...
const CHEMICALS = [
  { name: 'Sulphur', f: 'S', specs: ['Granular', 'Lump'], t: ['mfg', 'agri'],
  ...
  { name: 'Urea-A (technical)', f: 'CH&#8324;N&#8322;O', specs: ['Technical Grade A'], t: ['mfg'],
    origins: ['Uzbekistan'],
  ...
  { name: 'Caustic Soda', f: 'NaOH', specs: ['Solid', 'Liquid'], t: ['mfg', 'water'], origins: [],
  ...
  { name: 'Sodium Hypochlorite', f: 'NaOCl', specs: ['Disinfectant grade'], t: ['water'], origins: [],
  ...
  { name: 'Hydrochloric Acid', f: 'HCl', specs: ['Industrial grade'], t: ['mfg', 'water'], origins: [],
  ...
  { name: 'Liquid Chlorine', f: 'Cl&#8322;', specs: ['Bulk supply'], t: ['water'], origins: [],
  ...
  { name: 'Calcium Chloride', f: 'CaCl&#8322;', specs: ['Solid', 'Liquid'], t: ['mfg'], origins: [],
  ...
  { name: 'Sodium Sulphate', f: 'Na&#8322;SO&#8324;', specs: ['Detergent grade'], t: ['care', 'mfg'], origins: [],
  ...
  { name: 'LABSA', f: 'C&#8321;&#8328;H&#8330;&#8320;O&#8323;S', specs: ['96% purity'], t: ['care'], origins: [],
  ...
  { name: 'SLES', f: 'C&#8321;&#8322;H&#8322;&#8325;NaO&#8324;S', specs: ['70% concentration'], t: ['care'], origins: [],
  ...
  { name: 'Sulphuric Acid', f: 'H&#8322;SO&#8324;', specs: ['Industrial'], t: ['mfg', 'agri'], origins: [],
  ...
  { name: 'Formic Acid', f: 'CH&#8322;O&#8322;', specs: ['Preservative grade'], t: ['agri', 'mfg'], origins: [],
  ...
  { name: 'Acetex Plus', f: '&mdash;', specs: ['Multi-purpose'], t: ['mfg'], origins: [],
  ...
  { name: 'Aluminium Sulphate', f: 'Al&#8322;(SO&#8324;)&#8323;', specs: ['Water treatment'], t: ['water', 'mfg'], origins: [],
  ...
  { name: 'Iodine', f: 'I&#8322;', specs: ['Pharma grade'], t: ['mfg'], origins: ['Turkmenistan'],
  ...
  { name: 'Carbon Black', f: 'C', specs: ['Rubber grade'], t: ['mfg'], origins: ['Turkmenistan'],
  ...
  ...c, id: slug(c.name), cat: 'industrials',
  kind: c.t.map((k) => plain(SECTORS.find((s) => s[0] === k)[1])).join(' &middot; '),
}));
...
const CLASSES = [
  {
    key: 'fertilizers', no: '01', title: 'Fertilizers', href: 'fertilizers.html',
    tone: 'sand', items: FERTILIZERS,
    count: `${FERTILIZERS.length} grades`,
    blurb: 'Urea B (N46), potash, ammonia, ammonium nitrate and NPK &mdash; the nitrogen and compound grades that set yield.',
  },
  {
    key: 'polymers', no: '02', title: 'Polymers', href: 'polymers.html',
    tone: 'cyan', items: POLYMERS,
    count: `${POLYMERS.length} families`,
    blurb: 'Polyethylene across LDPE, HDPE, LLDPE and UHMWPE, polypropylene homo- and copolymer, plus performance additives.',
  },
  {
    key: 'industrials', no: '03', title: 'Industrial Chemicals', href: 'industrials.html',
    tone: 'cyan', items: CHEMICALS,
    count: `${CHEMICALS.length} grades`,
    blurb: 'Sulphur, caustic soda, sulphuric and hydrochloric acid, LABSA 96%, SLES 70%, carbon black, iodine and more.',
  },
];
...
const PAGES = [
  ['index.html', 'Home', 'Caspian and Central Asian commodity trading out of Dubai, across fertilizers, polymers and industrial chemicals.', 'globalex trading dmcc home landing'],
  ['products.html', 'Products', 'Every grade we trade, searchable in one place across all three commodity classes.', 'catalogue catalog grades search all products'],
  ['about.html', 'About', 'Established 2019 on honesty, integrity and trust. A DMCC freezone trading house in Jumeirah Lakes Towers, Dubai.', 'company history mission vision values leadership team dmcc freezone licence'],
  ['procedures.html', 'Trade Procedures', 'The ten contractual checkpoints between handshake and hull.', 'ncnd imfpa loi fco letter of credit lc proof of product pop performance bond sgs inspection incoterms cif fob shipment'],
  ['sustainability.html', 'Sustainability', 'Six commitments applied to product, partner and process decisions.', 'esg environment ethical sourcing transparency social impact'],
  ['careers.html', 'Careers', 'Open roles in Dubai, and how to apply.', 'jobs vacancies hiring apply cv digital marketing manager chemical engineer'],
  ['contact.html', 'Contact', 'The Dubai desk — address, phone, and the enquiry form. Replies within two business days.', 'enquiry quote rfq phone email address jlt cluster x map get in touch'],
  ['index.html#specifications', 'Specifications & MSDS', 'The controlled document register — safety, technical and origin sheets, delivered by secure link.', 'msds tds sds spec sheet datasheet safety data download documents'],
  ['privacy-policy.html', 'Privacy Policy', 'How we collect, use and safeguard personal information.', 'gdpr data protection cookies tracking'],
  ['terms-conditions.html', 'Terms & Conditions', 'The terms governing use of this website and our services.', 'legal terms conditions liability'],
];
...
function searchIndex() {
  ...
  CLASSES.forEach((cls) => {
    add({
      t: cls.title, k: 'Commodity class', u: cls.href, c: cls.key,
      d: plain(cls.blurb),
      h: plain(`${cls.title} ${cls.blurb} ${cls.count} commodity class products`),
    });
    cls.items.forEach((p) => {
      const words = [
        p.name, p.kind, p.body, p.alias || '', p.f || '',
        (p.specs || []).join(' '), (p.origins || []).join(' '),
      ].join(' ');
      add({
        t: plain(p.name), k: plain(cls.title), u: `${cls.href}#${p.id}`, c: cls.key,
        d: plain(p.body).slice(0, 150).replace(/\s+\S*$/, '') + '…',
        h: plain(words),
      });
    });
  });
  ...
  DOCS.forEach((d) => {
    add({
      t: `${d.title} (${d.kind})`, k: 'Document', u: 'index.html#specifications', c: 'documents',
      d: plain(d.summary).slice(0, 150).replace(/\s+\S*$/, '') + '…',
      h: plain(`${d.title} ${d.kind} ${d.code} ${d.sub} ${d.origin} ${d.summary} ${(d.specs || []).map((s) => s.join(' ')).join(' ')} document sheet pdf`),
    });
  });

  PAGES.forEach(([u, t, d, extra]) => {
    add({ t, k: 'Page', u, c: 'company', d: plain(d), h: plain(`${t} ${d} ${extra}`) });
  });
...
module.exports = {
  slug, plain, SECTORS,
  FERTILIZERS, POLYMERS, CHEMICALS, POLYMER_ORIGINS,
  CLASSES, PAGES, searchIndex,
};

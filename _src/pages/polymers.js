  ...
  glxStage(cv, function(x,w,h,t){
    ...
    if (mode === 'add'){
      ...
      for (var a=0;a<mol.length;a++) for (var b=a+1;b<mol.length;b++){
        ...
        if (d<lim){
          x.strokeStyle=RGBA('cyan',(.28*(1-d/lim)).toFixed(3));
      ...
      mol.forEach(function(p,k){
        ...
        x.fillStyle = p.sand ? RGBA('sandD',.5+pulse*.42) : RGBA('cyan',.45+pulse*.45);
    ...
    rows.forEach(function(c,ci){
      ...
      x.strokeStyle=RGBA('cyan',.42); x.lineWidth=1.4;
      ...
      if (c.branch){
        for (var s=0.12;s<1;s+=.19){
          ...
          x.fillStyle=RGBA('cyan',.6); x.fillRect(-1.5,-1.5,3,3); x.restore();
        }
      }
      ...
      for (var k=0;k<=beads;k++){
        ...
        var r=1.9+pulse*1.4;
        x.fillStyle=RGBA(pulse>.72?'signal':'cyan',.42+pulse*.5);
        x.fillRect(-r,-r,r*2,r*2); x.restore();
      }
    });
  });

...
.gatepage{min-height:100svh;display:grid;place-items:center;padding:1.5rem;
  background:radial-gradient(110% 70% at 50% 0,rgba(var(--cyan-rgb),.07),transparent 62%)}
...
input,select{width:100%;padding:.75rem .9rem;background:rgba(var(--void-rgb),.6);
...
.top{position:sticky;top:0;z-index:20;display:flex;align-items:center;gap:1rem;flex-wrap:wrap;
  padding:.85rem clamp(.9rem,2.4vw,1.7rem);background:rgba(var(--void-rgb),.95);
...
.top .tag{font-family:var(--f-mono);font-size:.645rem;letter-spacing:.2em;color:var(--cyan);
  border:1px solid rgba(var(--cyan-rgb),.3);padding:.25em .55em}
.top .sp{flex:1}
...
td{padding:.7rem .85rem;border-bottom:1px solid rgba(var(--hairline-rgb),.13);vertical-align:top}
tbody tr:hover{background:rgba(var(--cyan-rgb),.045)}
...
.pill.on{color:var(--cyan);border-color:rgba(var(--cyan-rgb),.4)}
.pill.off{color:var(--rose);border-color:rgba(var(--alert-rgb),.4)}
...
.bar::before{content:'';position:absolute;left:0;top:0;bottom:0;width:var(--w,0%);
  background:rgba(var(--cyan-rgb),.1);transition:width .6s var(--ease)}
...
.share-out{margin-top:1.2rem;border:1px solid rgba(var(--cyan-rgb),.34);background:rgba(var(--cyan-rgb),.06);
  padding:1rem 1.15rem}
.share-out.bad{border-color:rgba(var(--alert-rgb),.4);background:rgba(var(--alert-rgb),.07)}
...
.share-link input{flex:1;min-width:220px;font-family:var(--f-mono);font-size:.82rem;
  background:rgba(var(--void-rgb),.7)}
...
.dd-bd{position:absolute;inset:0;background:rgba(var(--scrim-rgb),.72);backdrop-filter:blur(5px)}
...
.ev-r{display:grid;grid-template-columns:auto 1fr auto;gap:.8rem;align-items:baseline;
  padding:.6rem .1rem;border-bottom:1px solid rgba(var(--hairline-rgb),.14)}
    ...
    <section>
      ...
      <div class="chart">
        ...
        <div class="legend">
          ...
          <span><i style="background:rgba(var(--hairline-rgb),.4)"></i>Views</span>
        </div>
      </div>
    </section>
